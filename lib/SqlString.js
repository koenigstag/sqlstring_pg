...
var MYSQL_CONFIG = {
  ...
  VALUE_PLACEHOLDER_REGEX: undefined, // default regex for placeholders is /\?+/g
};
...
function withConfig(customConfig = DEFAULT_CONFIG) {
  ...
  var CHARS_GLOBAL_REGEXP = CONFIG.CHARS_GLOBAL_REGEXP;
  ...
  SqlString.format = function format(sql, values, stringifyObjects, timeZone) {
    ...
    var chunkIndex         = 0;
    var placeholdersRegex  = CONFIG.VALUE_PLACEHOLDER_REGEX || /\?+/g;
    var result             = '';
    var valuesIndex        = 0;
    ...
    while (valuesIndex < values.length && (match = placeholdersRegex.exec(sql))) {
      ...
      var value = len === 2 && placeholdersRegex.source.includes('?')
